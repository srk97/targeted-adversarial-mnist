...
def build_image(x_feed):
	...
	write_jpeg(out, 'image_momentum.jpg')	
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		grads_l1_norm = tf.norm(grads,ord=1)
		...
		for j in range(SAMPLE_SIZE):
			...
			while sess.run(acc,feed_dict=feed_dict)!=1.0:
				...
				#Get the gradients from J_{\theta}(x,y_target) and the L1 norm
				cse = sess.run(cse_loss,feed_dict=feed_dict)
				grads_ = sess.run(grads,feed_dict=feed_dict)
				grads_l1_norm_ = sess.run(grads_l1_norm,feed_dict=feed_dict)

				#Accumulate the velocity vector with MU=1
				g = g + (grads_/grads_l1_norm_)

				#Update the image
				x_feed[j] = x_feed[j]-(EPSILON*(g[0]/LA.norm(g[0],ord=2)))		
